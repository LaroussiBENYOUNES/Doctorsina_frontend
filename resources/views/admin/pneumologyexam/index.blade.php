...
<p style="margin-top:15px; margin-left:40px">{!! link_to_route(config('quickadmin.route').'.pneumologyexam.create', 'New Pneumology Exam' , null, array('class' => 'btn btn-success')) !!}</p>
@endif

@if ($pneumologyexam->count())
<div class="x_panel">
...
<td>{{ isset($row->appoitement_id) ? link_to_route(config('quickadmin.route').'.appoitement.edit', $row->appoitement_id, array($row->appoitement_id), array('style' => 'background-color:  #e6f2ff')) : '--' }}</td>
                           ...
                           <td style="width:15%">
                            @if(Auth::user()->role_id == 10)
                                {!! link_to_route(config('quickadmin.route').'.pneumologyexam.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                            @else
                            {!! link_to_route(config('quickadmin.route').'.pneumologyexam.edit', Details, array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                            @endif
                                <button class="btn btn-danger btn-xs" onclick="deleteone({!!$row->id!!})" >
                                    delete
                                 </button>
                                 {!! Form::open(['route' => config('quickadmin.route').'.pneumologyexam.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
