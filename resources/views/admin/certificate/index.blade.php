...
<p style="margin-top:15px; margin-left:40px">{!! link_to_route(config('quickadmin.route').'.certificate.create', 'New Certificate' , null, array('class' => 'btn btn-success')) !!}</p>
@endif
                ...
                <tbody>
                    @foreach ($certificate as $row)
                        <tr>
                            ...
                            <td style="width:15%">
                            @if(Auth::user()->role_id == 10)
                                {!! link_to_route(config('quickadmin.route').'.certificate.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                            @else
                            {!! link_to_route(config('quickadmin.route').'.certificate.edit', Details, array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                            @endif
                            <button class="btn btn-danger btn-xs" onclick="deleteone({!!$row->id!!})" >
                                    delete
                                 </button>
                                 {!! Form::open(['route' => config('quickadmin.route').'.certificate.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
