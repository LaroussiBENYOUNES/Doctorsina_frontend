...
<div class="row">
    ...
    @if(auth()->user()->role_id==1)
        <h3 style="margin-left:10px" >Edit Vaccine</h3>
       @else
       <h3 style="margin-left:10px" >Vaccine Details</h3>
       @endif
        <br>
        @if ($errors->any())
...
<div class="form-group">
    ...
    @if(auth()->user()->role_id==1)
      ...
      {!! link_to_route(config('quickadmin.route').'.vaccine.index', Cancel, null, array('class' => 'btn btn-default')) !!}
    @else
    {!! link_to_route(config('quickadmin.route').'.vaccine.index', 'Go back', null, array('class' => 'btn',  'style' => 'background-color: #2A3F54; color: #FFFFFF; margin-left: 7px')) !!}
  @endif
    </div>
</div>
