...
<p style="margin-top:15px; margin-left:40px">{!! link_to_route(config('quickadmin.route').'.vaccine.create', 'New Vaccine', null, array('class' => 'btn btn-success')) !!}</p>
@endif
@if ($vaccine->count())
<div class="x_panel">
                        ...
                        @if((Auth::user()->role_id == 1))
                            ...
                            @if((Auth::user()->role_id == 1))
                                ...
                                <button class="btn btn-danger btn-xs" onclick="deleteone({!!$row->id!!})" >
                                 ...
                                 {!! Form::close() !!}
                            @else
                            {!! link_to_route(config('quickadmin.route').'.vaccine.edit', Details, array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                            @endif
                            </td>
                        </tr>
                    @endforeach
...
<script>
     function deleteone(id) {
           swal({
                title: 'Are you sure?',
                text: 'This vaccine and it`s details will be permanantly deleted!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
                }).then(function(value) {
                if (value) {
...
swal({
title: 'Are you sure?',
text: 'This vaccines and it`s details will be permanantly deleted!',
icon: 'warning',
buttons: ["Cancel", "Yes!"],
}).then(function(value) {
if (value) {
